...
p <- ggplot(df_oak, aes(x = Size_class_Reorder, y = Crown_score,  z = Burnt, fill = Burnt, color = Burnt )) +
  geom_boxplot() + theme_bw() + theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank())
p <- p + scale_fill_manual(values=c("green4","brown3"))
p <- p + scale_color_manual(values=c("darkgreen","brown4"))
p + theme_classic()  +
...
scheirerRayHare(Crown_score ~ Burnt + Size_class_Reorder,
                data = df_oak)
